    ...
    <div className="min-h-screen relative flex items-center justify-center px-4 py-10 text-white">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="rounded-2xl border border-white/15 bg-white/10 backdrop-blur-xl shadow-xl shadow-cyan-900/30 p-8">
          <h2 className="text-xl font-semibold mb-6 tracking-wide">Masuk Akun</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            >
              ...
              <span className="relative">{loading? 'Memproses...':'Login'}</span>
              <span className="absolute inset-0 rounded-full ring-1 ring-white/40 mix-blend-overlay" />
            </button>
          </form>
          <div className="flex items-center my-6">
            <div className="flex-1 h-px bg-white/10" />
            <span className="px-3 text-[10px] tracking-wider text-cyan-100/60 uppercase">atau</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>
          <GoogleLoginButton />
          <p className="mt-6 text-center text-xs text-cyan-100/70">Belum punya akun? <button onClick={()=>navigate('/register')} className="underline decoration-cyan-300/50 hover:decoration-cyan-200 underline-offset-4 font-medium">Register</button></p>
        </div>
