...
class TripController {
  static async create(req, res, next) {
    try {
      ...
      if (!interestsArr.length) {
        const err = new Error("Interests required");
        err.name = "BadRequest";
        throw err;
      }
      ...
      if (!finalCity || latitude == null || longitude == null) {
        const err = new Error(
          "Unable to resolve location (provide cityName or city)"
        );
        err.name = "BadRequest";
        throw err;
      }
      ...
      if (!startDate || !endDate) {
        const err = new Error("startDate and endDate required");
        err.name = "BadRequest";
        throw err;
      }
      ...
      if (process.env.NODE_ENV === "test") {
        ...
        try {
          weather = await getWeather(latitude, longitude, startDate, endDate);
        } catch {
  ...
  static async update(req, res, next) {
    try {
      ...
      const interestsChanged =
        JSON.stringify(interestsArr) !== JSON.stringify(trip.interest || []);
      const locationChanged =
        newCity !== trip.city || newLat !== trip.lat || newLon !== trip.lon;
      // Date handling
      ...
      const sd2 = new Date(newStartDate);
      const ed2 = new Date(newEndDate);
      if (isNaN(sd2) || isNaN(ed2) || ed2 < sd2) {
        const err = new Error("Invalid date range");
        err.name = "BadRequest";
        throw err;
      }
      ...
      const dateChanged =
        newStartDate !== trip.startDate || newEndDate !== trip.endDate;
  ...
  static async remove(req, res, next) {
    try {
      ...
      await trip.destroy();
      res.status(200).json({ message: "your trip has been deleted" });
    } catch (error) {
      next(error);
    }
  }
